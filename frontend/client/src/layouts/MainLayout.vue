<template>
  ...
  >
    ...
    <!-- 标题区域 -->
    <div
      class="w-full max-w-6xl mx-auto px-6 pt-10 sm:pt-8 pb-4 flex-shrink-0 relative z-10"
    >
      <div>
        <h1
          v-if="title"
    ...
    <div class="flex-1 flex flex-grow min-h-0 overflow-hidden relative z-10">
      ...
      <!-- 页面内容插槽 -->
      <div
        class="flex-1 flex-grow min-h-0 overflow-y-auto md:p-8 w-full flex justify-center"
      >
        <div
    ...
    >
      <div class="max-w-4xl mx-auto py-3 text-center">
        <p class="text-sm text-[var(--fe-text-secondary)]">
          我们的故事开始于:
          <span class="font-bold text-[var(--fe-text-primary)]">{{
            computedStartDate
          }}</span>
        </p>
      </div>

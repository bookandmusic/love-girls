...
export const albumApi = {
  ...
  async createAlbum(albumData: Omit<Album, "id">) {
    const response = await api.post<CreateOrUpdateAlbumResponse>(
      "/albums",
      albumData,
    );
    return response.data;
  ...
  async updateAlbum(id: number, albumData: Partial<Album>) {
    const response = await api.put<CreateOrUpdateAlbumResponse>(
      `/albums/${id}`,
      albumData,
    );
    return response.data;
  ...
  async addPhotos(albumId: number, photoIds: number[]) {
    const response = await api.post<{
      ...
      data: Photo[];
      message?: string;
    }>(`/albums/${albumId}/photos`, { photoIds });
    return response.data;
  },

  // 从相册中删除照片
  async deletePhoto(albumId: number, photoId: number) {
    const response = await api.delete<DeleteAlbumResponse>(

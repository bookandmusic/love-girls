<template>
  <div class="admin-layout flex flex-col h-screen overflow-hidden">
    ...
    <header class="admin-header flex-shrink-0 z-50">
      <div
        class="w-full px-4 sm:px-6 lg:px-8 min-h-16 flex items-center justify-between pt-0"
      >
        ...
        <div class="flex items-center space-x-3">
          ...
          >
            ...
            <span
              class="hidden sm:inline text-sm font-medium admin-text-primary"
            >
      ...
      >
        <template #content>
          <p class="text-center py-2 admin-text-secondary text-sm">
            确定要退出登录吗？
          </p>
        </template>
        <template #actions>
    ...
    <Transition name="sheet">
      <div v-if="showMobileMenu" class="fixed inset-0 z-[100] lg:hidden">
        ...
        <!-- Sheet 内容 -->
        <div
          class="absolute bottom-0 inset-x-0 bg-white/98 backdrop-blur-2xl rounded-t-[32px] px-6 pt-6 pb-safe-bottom shadow-[0_-8px_40px_rgba(0,0,0,0.15)] transition-transform"
        >
          <!-- 顶部把手 -->
...
const isActiveMenuItem = (itemPath: string) => {
  if (itemPath === "/content") {
    return route.path.startsWith("/content");
  }

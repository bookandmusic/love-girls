<template>
  <div class="w-full h-full flex flex-col overflow-hidden">
    <template v-if="places.length === 0 && !uiStore.loading">
      <div class="flex-1 flex flex-col items-center justify-center py-20">
        <BaseIcon name="place" size="w-16 h-16" color="text-gray-400" />
        <p class="text-xl font-bold mt-4 text-gray-700">暂无地点</p>
        <p class="text-sm mt-2 text-gray-500">还没有添加任何地点</p>
      </div>
    </template>
    <template v-else>
...
const totalPages = computed(
  () => Math.ceil(totalPlaces.value / pageSize.value) || 1,
);

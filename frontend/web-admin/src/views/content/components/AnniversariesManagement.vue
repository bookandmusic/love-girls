<template>
  <div class="w-full h-full flex flex-col overflow-hidden">
    <template v-if="anniversaries.length === 0 && !uiStore.loading">
      <div class="flex-1 flex flex-col items-center justify-center py-20">
        <BaseIcon name="anniversary" size="w-16 h-16" color="text-gray-400" />
        <p class="text-xl font-bold mt-4 text-gray-700">暂无纪念日</p>
        <p class="text-sm mt-2 text-gray-500">还没有添加任何纪念日</p>
      </div>
    </template>
    <template v-else>
...
const totalPages = computed(
  () => Math.ceil(totalAnniversaries.value / pageSize.value) || 1,
);
...
const loadAnniversaries = async (append = false) => {
  ...
  try {
    const response = await anniversaryApi.getAnniversaries(
      currentPage.value,
      pageSize.value,
    );
    ...
    totalAnniversaries.value =
      ...
      response.data.anniversaries.length;
  } catch (error) {
...
const saveAnniversary = async (anniversaryData: Anniversary) => {
  ...
  try {
    if (anniversaryData.id && anniversaryData.id > 0) {
      // 编辑现有纪念日
      const response = await anniversaryApi.updateAnniversary(
        anniversaryData.id,
        {

<template>
  <div class="w-full h-full flex flex-col overflow-hidden">
    <template v-if="moments.length === 0 && !uiStore.loading">
      <div class="flex-1 flex flex-col items-center justify-center py-20">
        <BaseIcon name="moment" size="w-16 h-16" color="text-gray-400" />
        <p class="text-xl font-bold mt-4 text-gray-700">暂无动态</p>
        <p class="text-sm mt-2 text-gray-500">还没有发布任何动态</p>
      </div>
    </template>
    <template v-else>
      <div ref="scrollContainer" class="flex-1 min-h-0 overflow-y-auto pr-2">
        ...
        >
          <div
            v-if="uiStore.loading && moments.length > 0"
          ...
          >
            ...
            ></div>
            <span
              class="text-xs font-semibold admin-text-secondary tracking-wide"
              >正在加载...</span
            >
          </div>
          <div
    ...
    >
      ...
      <template #actions>
        <div class="w-full flex">
          <div class="flex-1 text-center cursor-pointer" @click="cancelDelete">
            取消
          </div>
          <div
    ...
    >
      ...
      <template #actions>
        <div class="w-full flex">
          <div class="flex-1 text-center cursor-pointer" @click="cancelToggle">
            取消
          </div>
          <div
            class="w-1/2 border-l border-gray-300 text-center cursor-pointer text-red-500"
...
const totalPages = computed(
  () => Math.ceil(totalMoments.value / pageSize.value) || 1,
);
...
const performTogglePublic = async () => {
  ...
  try {
    ...
    const momentIndex = moments.value.findIndex(
      (u) => u.id === confirmToggle.value!.id,
    );
    if (momentIndex !== -1) {
    ...
    // 更新本地状态
    showToast(
      confirmToggle.value.isPublic ? "动态已设为公开" : "动态已设为私密",
      "success",
    );
  } catch {

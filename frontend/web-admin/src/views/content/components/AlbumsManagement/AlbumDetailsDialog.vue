    ...
    <template #content>
      <div class="flex flex-col min-h-0">
        ...
        <div class="flex-1 overflow-y-auto min-h-0" @scroll="handleScroll">
          <div
            class="px-2 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 py-1"
          >
            <div
            ...
            >
              <img
                ...
                :class="{
                  'ring-2 ring-indigo-500': selectedCoverId === photo.id,
                }"
              />
              <!-- 删除按钮 -->
              <button
...
watch(
  ...
  (newAlbum) => {
    if (newAlbum && newAlbum.coverImage) {
      const coverPhoto = props.photos.find(
        (p) => p.id === newAlbum.coverImage?.id,
      );
      if (coverPhoto) {
...
watch(
  ...
  () => {
    if (props.album && props.album.coverImage) {
      const coverPhoto = props.photos.find(
        (p) => p.id === props.album?.coverImage?.id,
      );
      if (coverPhoto) {
        selectedCoverId.value = coverPhoto.id;
      } else {

<template>
  <div class="h-full flex flex-col overflow-hidden bg-[var(--fe-bg-gray)]/30">
    <!-- 顶部玻璃导航 -->
    <div class="sticky top-0 z-20 glass-thick p-4 border-b border-white/20 flex items-center">
      <button
        @click="onBack"
    ...
    >
      ...
      <!-- 空状态 -->
      <div v-else-if="!loading" class="flex-1 flex flex-col items-center justify-center py-20">
        <BaseIcon name="photo-heart" size="w-24" color="var(--fe-text-secondary)" />
        <p class="text-xl font-bold mt-4 text-[var(--fe-text-primary)]">暂无照片</p>
        <p class="text-md mt-2 text-[var(--fe-text-secondary)]">期待分享第一张照片</p>
      </div>
      ...
      <div v-if="loading || hasMore" class="py-10 flex justify-center">
        <div v-if="loading" class="flex items-center space-x-2 text-[var(--fe-text-secondary)]">
          ...
          ></div>
          <span class="text-xs font-bold uppercase tracking-widest">正在加载...</span>
        </div>
        <div

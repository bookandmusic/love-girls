...
**过滤示例**：
- `filter=name:like:旅行` - 搜索名称包含"旅行"的相册

### 请求示例（curl）

```bash
# 基础分页查询
curl -X GET "http://localhost:8182/api/v1/albums?page=1&size=10"

# 按名称升序排序
curl -X GET "http://localhost:8182/api/v1/albums?page=1&size=10&sort_by=name&order=asc"

# 搜索名称包含"旅行"的相册
curl -X GET "http://localhost:8182/api/v1/albums?page=1&size=10&filter=name:like:旅行"
```
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.2.0 | 2026-03-13 | 新增：列表接口支持排序和过滤功能，新增 sort_by、order、filter 参数 |

...
获取纪念日列表，支持分页、排序和过滤查询。

### 请求信息

- **接口路径**: `GET /api/v1/anniversaries`
...
| 参数名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| sort_by | string | 否 | created_at | 排序字段，可选值：`date`、`created_at` |
...
```bash
# 基础分页查询
curl -X GET "http://localhost:8182/api/v1/anniversaries?page=1&size=10"

# 按日期升序排序
curl -X GET "http://localhost:8182/api/v1/anniversaries?page=1&size=10&sort_by=date&order=asc"

# 按创建时间降序排序
curl -X GET "http://localhost:8182/api/v1/anniversaries?page=1&size=10&sort_by=created_at&order=desc"
```
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.3.0 | 2026-03-13 | 新增：列表接口支持排序和过滤功能，新增 sort_by、order、filter 参数 |

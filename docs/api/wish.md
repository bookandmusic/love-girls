...
- **需要认证**: 否（未认证用户只能查看已审核的祝福，已认证用户可以查看所有祝福）
...
| 参数名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| sort_by | string | 否 | created_at | 排序字段，可选值：`created_at` |
...
| approved | boolean | 否 | 是否只获取已审核的祝福（仅已认证用户有效） |

**支持的过滤字段和操作符**：

| 字段 | 支持的操作符 | 说明 |
|------|-------------|------|
| approved | eq | 按审核状态过滤，值：`true`/`false` |

**过滤示例**：
- `filter=approved:eq:true` - 只查看已审核的祝福

### 请求示例（curl）

```bash
# 基础分页查询
curl -X GET "http://localhost:8182/api/v1/wishes?page=1&size=10"

# 按创建时间升序排序
curl -X GET "http://localhost:8182/api/v1/wishes?page=1&size=10&sort_by=created_at&order=asc"

# 只查看已审核的祝福（已认证用户）
curl -X GET "http://localhost:8182/api/v1/wishes?page=1&size=10&filter=approved:eq:true" \
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.4.0 | 2026-03-13 | 新增：列表接口支持排序和过滤功能，新增 sort_by、order、filter 参数 |
| 1.3.0 | 2026-02-02 | 更新：列表查询和创建祝福不需要认证，删除单个祝福查询接口 |
| 1.2.0 | 2026-01-30 | 重命名：将Wish API重命名回Wish API，以匹配前端实现 |

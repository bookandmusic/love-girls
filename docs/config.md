...
| `SERVER_INTERNAL_URL` | 内网地址（ImageProxy 访问 Gin 用） | - | 如 `http://gin:8181` |
...
| `STORAGE_S3_PUBLIC_URL` | S3 公共访问地址（可选） | - |
...
| `STORAGE_WEBDAV_PUBLIC_URL` | WebDAV 公共访问地址（可选） | - |
| `STORAGE_WEBDAV_AUTH_USERNAME` | WebDAV 认证用户名 | - |
| `STORAGE_WEBDAV_AUTH_PASSWORD` | WebDAV 认证密码 | - |

### 图片代理配置

图片代理服务（如 imgproxy / thumbor）用于生成缩略图和图片处理。

| 环境变量 | 说明 | 默认值 |
|---------|------|--------|
| `IMAGE_PROXY_INTERNAL_URL` | 内网地址（Gin 转发缩略图用） | - |
| `IMAGE_PROXY_PUBLIC_URL` | 公开地址（前端直接访问，可选） | - |

**使用场景**：
- `INTERNAL_URL`: Gin 服务转发缩略图请求时使用，走内网
- `PUBLIC_URL`: 前端直接访问图片代理时使用，走公网
...
STORAGE_S3_PUBLIC_URL=https://s3.example.com
```
...
STORAGE_WEBDAV_PUBLIC_URL=https://cdn.example.com
...
```bash
# Gin 转发缩略图请求走内网
IMAGE_PROXY_INTERNAL_URL=http://imgproxy:8080
...
image_proxy:
  internal_url: http://imgproxy:8080
